...
objects described below or subclass :class:`terms.Term` to implement a custom
kernel. Subclasses should overload the
:func:`terms.Term.get_real_coefficients` and
:func:`terms.Term.get_complex_coefficients` methods to provide custom
...
Since ``Term`` objects implement the modeling protocol, for the kernel
...
This section describes the low-level interface to the C++ implementation of
the celerite algorithm. These methods do most of the heavy lifting but most
uses shouldn't need to call these directly. This interface was built using
`pybind11 <http://pybind11.readthedocs.io/>`_.
