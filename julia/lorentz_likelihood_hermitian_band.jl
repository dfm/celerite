function lorentz_likelihood_hermitian_band(alpha::Vector{Float64},beta::Vector{Complex{Float64}},
...
# Equation for r (59):
      ...
      if i < n-1
        ...
        jcol = 2p+3
        aex[irow+p+j,jcol] = exp(-real(beta[j])*(t[i+2]-t[i+1]))*complex(cos(phi),-sin(phi))
      end
    end
  end
end
