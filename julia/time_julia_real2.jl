...
function log_like_derivative_wrapper(p,x,t,y)
  ...
  function log_like(x)
    ...
    nex::Int64 = (2p+1)*n-2p
    aex_real1 = zeros(eltype(x),nex,2*p+3)
    aex_real2 = zeros(eltype(x),nex,2*p+3)
    m1::Int64 = p+1
    al_small_real1 = zeros(eltype(x),nex,m1)
    al_small_real2 = zeros(eltype(x),nex,m1)
...
beta_imag = [0.0,omega1,-omega1, omega2, -omega2, omega3, -omega3, omega4, -omega4]
...
# Now use Ambikarasan O(N) method:
  ...
  m1_real = p_real+1
  aex_real1 = zeros(Float64,nex_real,width_real)
  aex_real2 = zeros(Float64,nex_real,width_real)
  al_small_real1 = zeros(Float64,nex_real,m1_real)
  al_small_real2 = zeros(Float64,nex_real,m1_real)
