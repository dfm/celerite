...
function compile_matrix_symm01(alpha,beta_real,beta_imag,w,t,y)
...
for k=2:n
  for j=1:p
    variables[(k-2)*(4p+1)+1+(j-1)*2+1] = string(" r^R_{",k,",",j,"}")
    variables[(k-2)*(4p+1)+1+(j-1)*2+2] = string(" r^I_{",k,",",j,"}")
  end
  for j=1:p
    variables[(k-2)*(4p+1)+1+2p+(j-1)*2+1] = string(" l^R_{",k,",",j,"}")
    variables[(k-2)*(4p+1)+1+2p+(j-1)*2+2] = string(" l^I_{",k,",",j,"}")
  end
  variables[(k-1)*(4p+1)+1] = string(" x_",k)
