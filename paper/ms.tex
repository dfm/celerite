...
given by $[K_\bvec{\alpha}]_{ij} = k_\bvec{\alpha}(\bvec{x}_i,\,\bvec{x}_j)$.
...
$k_\bvec{\alpha}(\bvec{x}_i,\,\bvec{x}_j)$ and it has several limitations.
The method can only be applied to one-dimensional datasets.
When we say ``one-dimensional'' here, it means that the \emph{input
coordinates} $\bvec{x}_i$ are scalar, $\bvec{x}_i \equiv t_i$.\footnote{We are
...
This means that the function $k_\bvec{\alpha}(t_i,\,t_j)$ is only a function
of $\tau_{ij} \equiv |t_i - t_j|$.


\section{The genrp model}

\subsection{Gaussian processes with Lorentzian power spectra}

Parameters:
\begin{eqnarray}\eqlabel{kernel}
k(\tau) &=& \sum_{j=1}^p \left[
    a_j\,\exp\left(-c_j\,\tau\right)\,\cos\left(-d_j\,\tau\right) +
    b_j\,\exp\left(-c_j\,\tau\right)\,\sin\left(-d_j\,\tau\right)
\right]
\end{eqnarray}
where $\tau = |\Delta t|$.
...
\begin{eqnarray}
k(\tau) &=& \sum_{j=1}^p \left[
    \frac{1}{2}(a_j + i\,b_j)\,\exp\left(-(c_j+i\,d_j)\,\tau\right) +
    \frac{1}{2}(a_j - i\,b_j)\,\exp\left(-(c_j-i\,d_j)\,\tau\right)
\right]\quad.
\end{eqnarray}

The power spectral density is computed by taking the Fourier transform of
\eq{kernel} to find
\begin{eqnarray}
...
\right]
\end{eqnarray}
a pair of Lorentzian distributions with width $c_j$ centered on
$\omega = \pm d_j$.


\citep{Rybicki:1995, Ambikasaran:2015}

\subsection{Generalization to quasi-periodic covariance}

Our method.

%The general ACF is given by
%\begin{equation}
%k(\tau) = a_r e^{-b_r \tau} \cos{(b_r \tau)} + a_i e^{-b_r \tau} \sin{(b_r \tau)}.
%\end{equation}
%which has a power spectrum given by
%\begin{equation}
%P(\omega) = P_0 \frac{(a_ib_i+a_rb_r)(b_i^2+b_r^2)+(a_rb_r-a_ib_i)\omega^2}{(\omega^2-(b_i^2+b_r^2))^2 + 4b_r^2\omega^2}.
%\end{equation}
%The power spectrum should be non-negative since it is the absolute square of the Fourier transform of the noise.
%Note that the denominator is always non-negative, so the power spectrum is non-negative if $a_ib_i+a_rb_r \ge 0$ and
%$a_rb_r-a_ib_i \ge 0$.

\subsection{Relation to stochastically-driven, damped simple harmonic oscillator}

Intrinsic stellar variability consists primarily of random fluctuations driven by
convection, oscillations excited by convection, and rotational variability.  These first
two types of variability may be well-described by a model of a damped simple-harmonic oscillator
driven by stochastic variability \citep{1990ApJ...364..699A}.  The governing differential equation is
\begin{equation}
y^{\prime\prime}+ \frac{\omega_0}{Q} y^\prime + \omega_0^2 y = \epsilon,
\end{equation}
where $y(t)$ is a displacement, $^{\prime}$ represents a time derivative,
$\omega_0$ is the frequency of an undamped oscillator, $Q$ is the quality factor
of the oscillation, and $\epsilon(t)$ is a stochastic driving force.

In the limit of an infinite time series and white random forcing, the noise-free power
spectrum of this equation is given by:
\begin{equation}
P(\omega) = P_0 \sqrt{\frac{2}{\pi}} \frac{\omega_0^4}{(\omega^2-\omega_0^2)^2 + \omega_0^2\omega^2/Q^2},
\end{equation}
where $P_0$ is a normalization constant.

% Check to see that signs match
This power spectrum matches that of our basis kernel if
\begin{eqnarray}
a_j &=& P_0  \omega_0 Q\\
b_j &=& a_j \left(4Q^2-1\right)^{-1/2}\\
c_j &=& \frac{\omega_0}{2Q}\\
d_j &=& \omega_0 \left(1-\frac{1}{4Q^2}\right)^{1/2},
%\omega_0^2 &=& c_j^2+d_j^2,\\
%a_jc_j-b_jd_j &=& 0,\\
%\omega_0^2&=& b_jd_j+a_jc_j ,\\
%\omega_0^2/Q^2&=& 4c_j^2.
\end{eqnarray}
for $Q \ge \frac{1}{2}$ and $j=p=1$.  This gives a kernel with dependence
\begin{eqnarray}\eqlabel{sho_kernel}
%k(\tau) &=& P_0  \omega_0 Q e^{-\frac{\omega_0 \tau}{2Q}} \left[\cos{\lambda}+\frac{1}{\left(4Q^2-1\right)^{1/2}}
k(\tau) &=&  P_0  \omega_0 Q e^{-\frac{\omega_0 \tau}{2Q}} \cos{\left(\lambda-\phi\right)},\\
\lambda &=& \left(1-\frac{1}{4Q^2}\right)^{1/2}\omega_0\tau,\\
\tan{\phi} &=& \left(4Q^2-1\right)^{-1/2}.
\end{eqnarray}
The power spectrum can be normalized by $P_0\omega_0 Q$ in amplitude and $\omega_0$ in
frequency to yield a spectral shape that only depends upon $Q$.  This is plotted
in Figure \ref{sho_psd} for various values of $Q$.
Note that the oscillation frequency that appears in the kernel, $d_j$, differs
from that of the simple-harmonic oscillator, $\omega_0$, due to the damping
frequency, $c_j$.  This power spectrum has
a peak frequency given by $\omega_{max} = \omega_0\sqrt{1-1/(2Q^2)} = d_j$, which is
greater than zero for $Q > 1/\sqrt{2}$.

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.75\textwidth]{./approximate_kernels/sho.png}
\caption{Normalized power spectra of the stochastically-driven, damped simple
harmonic oscillator. The dotted black line shows the peak frequency, $d_j = \omega_0 (1-1/(2Q)^2)^{1/2}$.}
\label{sho_psd}
\end{center}
\end{figure}
...
\begin{itemize}
\item $Q=1/2$:  This corresponds to a critically damped harmonic oscillator, which results in a power
spectrum $\propto 1/(x^2+1)^2$, where $x=\omega/\omega_0$.
This matches the power spectrum of the Mat\'ern 3/2 kernel, and can be represented by the following
autocovariance function:
\begin{equation}
k(\tau) = a_j e^{-c_j \tau} \left[\cos{(d_j \tau)} + \frac{c_j}{d_j}\sin{(d_j \tau)}\right],
% Check to see that signs match in this equation.
\end{equation}
in the limit that $d_j \rightarrow \infty$, corresponding to $\phi=\pi/2$, for which the second
term has a coefficient that diverges, but with large values of $d_j$, this may approximate the
Mat\'ern kernel well.  Note that this power spectrum peaks at $\omega=0$.
\item $Q=1/\sqrt{2}$:  This results in a power spectrum $\propto 1/(x^4+1)$, which is a functional
form used to fit the Solar granulation power spectrum by \cite{2009A&A...495..979M}.  This kernel has
dependence
\begin{equation}
k(\tau) = a_j e^{-c_j\tau} \left[\cos{(d_j\tau)}+\sin{(d_j\tau)}\right],
\end{equation}
corresponding to $\phi = \pi/4$.
Despite the sinusoidal dependence of the kernel, the strong damping causes the power spectrum to
peak at zero frequency; such a low quality oscillator doesn't even display quasi-periodic behavior.
\item $Q \gg 1$: This represents a high-quality oscillator which approaches a narrow peak.
The kernel approaches
\begin{equation}
k(\tau) = P_0 \omega_0 Q e^{-\omega_0\tau/(2Q)} \cos{\left(\omega_0\tau\right)},
\end{equation}
for $Q \gg 1$.  The power spectrum approaches
\begin{equation}
P(\omega) \approx P_0 \frac{Q^2}{1-1/(4Q^2)+4x_{max}^2\left(x-x_{max}\right)^2Q^2},
\end{equation}
where $x_{max}=\sqrt{1-1/(2Q^2)}$.  This shape approaches a Lorentzian for $x \approx x_{max}$ with
a width of $1/(Qx_{max})$ and a peak height $\propto Q^2$.
\end{itemize}

The stochastically-driven, damped simple-harmonic oscillator can then describe a wide range of intrinsic
stellar variability, with low $Q \approx 1$ representing granulation noise, and high $Q \gg 1$ representing
asteroseismic variability.  Taking the sum over oscillators with various $Q, P_0,$ and $\omega_0$ can
give an accurate accounting of the power spectrum of stellar variability with $Q \ge \frac{1}{2}$ and
$P_0 >0$.  As this kernel is exactly described by the exponential kernel, this functional form
is tractable to solving in $\mathcal{O}(N)$ operations, as described next.
...
In the case of $p$ damped sinusoids we can check for negative values of the PSD by solving for the
roots of the power spectrum, abbreviating with $z = \omega^2$:
\begin{equation}
P(\omega)=  \sum_{j=1}^{p} \frac{q_j z + r_j}{z^2+s_jz + t_j} = 0
\end{equation}
where
\begin{eqnarray}
q_j &=& a_jc_j-b_jd_j\\
...
\end{equation}
which is a polynomial with order $p_{ord}=2(p-1)+1$.  With $p=2$, this yields a cubic equation
...
%\begin{equation}
%k_j(\tau) = a_0 \delta(\tau) + \sum_{j=1}^{N_{kernel}} a_j \exp{(-b_j \tau)} \cos{(-d_j \tau)} + b_j \exp{(-b_j \tau)} \sin{(-d_j \tau)},
%\end{equation}
