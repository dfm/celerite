...
The method is fast and interpretable, with a wide range of potential
applications within astronomical data analysis and beyond.
...
\subsection{Scalable computation of celerite models}
...
matter~--~one must choose a kernel.
A detailed discussion of how to choose a kernel is beyond the scope of this
paper and the interested reader is directed to Chapter~5 of
\citet{Rasmussen:2006} a general introduction, but we include a summary and
some suggestions specific to \celerite\ here.
We argue that the most satisfying method of choosing a kernel is to use
physical motivation and the discussion in \sect{sho} provides an opportunity
for justified model specification when \celerite\ is applied to parameter
estimation from stellar light curves.
However, it is not always possible to rely on physics to specify the model
and, in these cases, it is useful to have other tools at one's disposal.
One option is to compute the evidence integral or Bayes factor for a set of
models and choose the model with the highest evidence.
Another option is to perform cross validation and select the model with the
best predictive performance.
For continuous autoregressive moving average models (we return to these in
\sect{compare}), \citet{Kelly:2014} recommend the Bayesian Information
Criterion (BIC) as a method for selecting the order of the model.
This is a total hack, but I guess that you could do it if you wanted.
...
Multiplication of the full covariance matrix times a vector can be accomplished in $\mathcal{O}(RN)$
...
\begin{eqnarray}
f^+_{n,j} &=& \phi_{n+1,j} \left(f^+_{n+1,j} + \tilde U_{n+1,j} z_{n+1}\right)\\
f^-_{n,j} &=& \phi_{n,j} \left(f^-_{n-1,j} + \tilde V_{n-1,j} z_{n-1}\right)\\
y_n &=& A_{n,n} z_n + \sum_{j=1}^R \left[ \tilde V_{n,j} f^+_{n,j} + \tilde U_{n,j} f^-_{n,j}\right],
\end{eqnarray}
where $f^+_{N,j} = 0$ and $f^-_{1,j} = 0$ for all $j=1,...,R$.  The recursion requires
two sweeps, a sweep downward from $n=N-1$ to $n=1$ to compute $f^+_{n,j}$ and a sweep upwards
from $n=2$ to $n=N$ to compute $f^-_{n,j}$.
...
kernel times $\bvec{z}$ may be accomplished in $\mathcal{O}(RN)$ steps with this multiplication algorithm.
