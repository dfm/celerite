...
since the computational cost commonly scales as the cube of the number of data points,
...
application is generally limited to small datasets, $N \approx 10^3$, because the cost of
...
$R=J_\mathrm{real} + 2\,J_\mathrm{complex}=2J-J_{real}$.}
...
\Figure{astero_psd} shows a comparison of
the power spectrum estimated with a Lomb-Scargle periodogram 
with the power spectrum inferred with \celerite.  Despite the much smaller 
one-month sequence of data used in the \celerite\ analysis, the power spectrum 
inferred with \celerite\ matches well the periodogram estimated from
the full four years of Kepler data.
...
\caption{The same as \Figure{benchmark}, but using an optimized
