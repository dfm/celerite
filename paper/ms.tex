...
\begin{eqnarray}
k(\tau) &=& \lim_{f \to 0}\,
    \frac{1}{2}\,S_0\,\omega_0\,e^{-\omega_0\,\tau}\,
    \left[\cos(f\,\tau) + \frac{\omega_0}{f}\,\sin(f\,\tau)\right] \\
&=& \frac{1}{2}\,S_0\,\omega_0\,e^{-\omega_0\,\tau}\,[1-\omega_0\,\tau] \quad.
\end{eqnarray}
This covariance function is also known as the Mat\'ern-3/2 function
\citep{Rasmussen:2006}.
}

{\item Finally, in the limit $Q \to \infty$, \eq{sho-psd} becomes
\begin{eqnarray}
S(\omega) =
\sqrt{\frac{2}{\pi}}\,\frac{S_0}{\left[(\omega/\omega_0)^2-1\right]^2} \quad.
\end{eqnarray}}
...
The stochastically-driven, damped simple-harmonic oscillator can then describe
a wide range of intrinsic stellar variability, with low $Q \approx 1$
representing granulation noise, and high $Q \gg 1$ representing asteroseismic
variability.
Taking the sum over oscillators with various $Q, S_0,$ and $\omega_0$ can give
an accurate accounting of the power spectrum of stellar variability with $Q
\ge \frac{1}{2}$ and $S_0 >0$.
As this kernel is exactly described by the exponential kernel, this functional
form is tractable to solving in $\mathcal{O}(N)$ operations, as described
next.
