...
%\begin{equation}
%K(\tau) = a_r e^{-b_r \vert\tau\vert} \cos{(b_r \vert\tau\vert)} + a_i e^{-b_r \vert\tau\vert} \sin{(b_r \vert\tau\vert)}.
%\end{equation}
...
\begin{equation}
P(\omega) = P_0 \frac{\omega_0^4}{(\omega^2-\omega_0^2)^2 + \omega_0^2\omega^2/Q^2},
\end{equation}
...
\end{eqnarray}
The power spectrum can be normalized by $P_0$ in amplitude and $\omega_0$ in
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=\textwidth]{./approximate_kernels/sho.png}
...
\begin{itemize}
\item $Q=1/2$:  This results in a power spectrum $\propto 1/(x^2+1)^2$, where $x=\omega/\omega_0$.
This matches the power spectrum of the Mat\'ern 3/2 kernel, and can be represented by the following
autocovariance function:
\begin{equation}
K(\tau) = a_j e^{-c_j \vert\tau\vert} \left(\cos{(d_j \vert\tau\vert)} + \frac{c_j}{d_j}\sin{(d_j \vert\tau\vert)}\right),
% Check to see that signs match in this equation.
\end{equation}
in the limit that $b_i \rightarrow \infty$, for which the second term has a coefficients that diverges,
but with large values of $b_i$, this may approximate the Mat\'ern kernel well.  Note that this
power spectrum peaks at $\omega=0$.
...
dependence
\begin{equation}
K(\tau) = a_r e^{-b_r\vert\tau\vert} \left(\cos{(b_i\vert\tau\vert)}+\sin{(b_i\vert\tau\vert)}\right).
\end{equation}
Despite the sinusoidal dependence of the kernel, the strong damping causes the power spectrum to
peak at zero frequency; such a low quality oscillator doesn't display even quasi-periodic behavior.
\item $Q \gg 1$: This represents a high-quality oscillator which approaches a narrow peak.  The
power spectrum approaches
\begin{equation}
...
a width of $1/(2Qx_{max})$ and a peak height $\propto Q^2$.
\end{itemize}

The stochastically-driven, damped simple-harmonic oscillator can then represent a wide range of intrinsic 
stellar variability, with low $Q \approx 1$ representing granulation noise, and high $Q >> 1$ representing 
asteroseismic variability.
