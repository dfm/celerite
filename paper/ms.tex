...
where $(D^{1/2})_{ii} = \sqrt{D_{ii}}$.  Also note that the first term in $-2\ln{\mathcal{L}}$
...
&=& \bvec{q}^T \bvec{q}.
\end{eqnarray}
This last quantity is simply the dot product of an $N$-dimensional vector of independent standard normal random variables with itself,
...
%\begin{eqnarray}
%\bar f_{*,i} &=& Re\left[\sum_j e^{-(c_j+id_j)( t_{*,i}-t_{*,i-1})} \sum_{n=1}^{n_0} (a_j+ib_j) e^{-(c_j+id_j)( t_{*,i-1}-t_n) } z_n\right. \\
%&+&\left.
%\sum_j e^{-(c_j+id_j)( t_{*,i+1}-t_{*,i})} \sum_{n=n_0+1}^{N} (a_j+ib_j) e^{-(c_j+id_j)( t_n-t_{*,i+1}) } z_n\right]
%\end{eqnarray}
...
\input{figures/transit/transit-params}
