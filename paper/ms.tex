...
\end{eqnarray}
that we can solve for $X$. \response{X was used to represent independent
...
\right]
\end{eqnarray}
which, for large values of $\omega$, falls off exponentially. \response{$\alpha$
...
\right]
\end{eqnarray}
for $a>0$, $b>0$, and $c>0$. \response{a,b,c already have different meaning
in celerite kernel.}
...
are shared nuisance parameters. \response{A was used for diagonal of
covariance matrix.}
...
\response{Table 5 has two parameter with name "b":  the celerite coefficient,
...
\response{The D in Table 6 may be confused with the D used in the Cholesky solver.}
