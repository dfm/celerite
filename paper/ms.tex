...
\item {\bf Exponential kernel.} This is also known as a Mat\'ern 1/2 kernel, and is exactly
...
\item {\bf Squared exponential.} The squared exponential or Gaussian kernel, $e^{-(\tau/\tau_0)^2/2}$, 
has the behavior of declining steeply at large time separation, is an even function, and has $c_1$ = 0.  
An infinite number of cosine kernels may be used to approximate a Gaussian kernel, but any
finite combination of cosine kernels leads to a negative amplitude at finite $\tau$.  Thus,
in general it is difficult to model a squared exponential with sums of damped cosine terms due
to the heavier tail.  However, the Mat\'ern kernel may be used to approximate it on short
timescales.
...
values of $p$, the Mat\'ern kernel may be approximated well by the sums of
exponential kernels, described next.
...
turns out to be an excellent description of stellar activity and granulation, and thus the approximate
...
\item {\bf Mat\'ern 5/2 kernel.}  The Mat\'ern 5/2 kernel is given
by
\begin{equation}
C_{5/2}(x) = \sigma^2 (1+y+y^2/3)e^{-y},
\end{equation}
where $y = \sqrt{5}x$.  The Fourier transform is:
\begin{equation}
\hat K_{5/2}(\omega) = \frac{2\tau_0 10^{5/2}}{3\pi^{1/2}} (5+\tau_0^2\omega^2)^{-3}.
\end{equation}
This kernel may be approximated by the sum of three exponential
kernels, and has a steeper dependence at large frequencies, $f^{-6}$.  We aren't aware of a stellar
power spectrum that requires such a steep spectrum, and thus we forgo implementation of this kernel.
\end{itemize}
