...
for $K^{-1}\,\bvec{y}$ and $\det K$ in $\mathcal{O}(N)$ operations.
...
$K^{-1}\,\bvec{y}$, and subsequently calculate the marginalized likelihood in
\eq{gp-likelihood}.
...
For this example, we will simulate two datasets using known GP models with
kernels outside of the support of a \celerite\ process.
We will use these simulations to demonstrate that, at least in these cases,
valid inferences can be made about the physical parameters of the model even
if the effective covariance function is known to be ``wrong''.
