...
covariance function given by \eq{celerite-kernel-complex}.
...
u_{n,j} &=& \phi_{n+1,j}\,u_{n+1,j} + \frac{a_j}{2}\,x_n + \psi_{n+1,j}\,v_{n+1,j}\\
v_{n,j} &=& \phi_{n+1,j}\,v_{n+1,j} - \frac{b_j}{2}\,x_n - \psi_{n+1,j}\,u_{n+1,j}
\end{eqnarray}

\begin{eqnarray}
\sum_{j=1}^J \left[\frac{a_j}{2}\,g_{n,j}+\frac{b_j}{2}\,h_{n,j}\right]
