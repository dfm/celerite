...
\todo{This method is a generalization of a method developed by
...
\todo{More precisely, the matrix can be embedded in an equivalent higher-dimensional
...
describe the \todo{(delete) previously published} scalable method
...
$\bvec{\alpha}$ for a given dataset $(\bvec{y},\,X)$ \todo{(Why
...
\end{eqnarray}
that we can solve for $X$. \todo{X was used to represent independent
...
\right]
\end{eqnarray}
which, for large values of $\omega$, falls off exponentially. \todo{$\alpha$
...
\right]
\end{eqnarray}
for $a>0$, $b>0$, and $c>0$. \todo{a,b,c already have different meaning
in celerite kernel.}
...
are shared nuisance parameters. \todo{A was used for diagonal of
covariance matrix.}
...
\todo{Table 5 has two parameter with name "b":  the celerite coefficient,
...
\todo{The D in Table 6 may be confused with the D used in the Cholesky solver.}
