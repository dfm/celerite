...
\begin{eqnarray}
\mathcal{Q} &=&{\bf y}^T K_\bvec{\alpha}^{-1} {\bf y}\\
&=& {\bf y}^T \left(K_\bvec{\alpha}^{-1/2}\right)^T K_\bvec{\alpha}^{-1/2} {\bf y}\\
&=& {\bf q}^T K_\bvec{\alpha}^{1/2} \left(K_\bvec{\alpha}^{-1/2}\right)^T K_\bvec{\alpha}^{-1/2} K_\bvec{\alpha}^{1/2} {\bf q}\\
&=& {\bf q}^T {\bf q}.
\end{eqnarray}
...
\begin{eqnarray}
\bar f &=& K({\bf t},{\bf t}) \left[K({\bf t},{\bf t})+\sigma^2{\bf I}\right]^{-1} {\bf y},\\
&=& K({\bf t},{\bf t}) {\bf b}
\end{eqnarray}
where ${\bf y}$ is the vector of measurements and ${\bf b}$ may be computed from the Cholesky
decomposition as described above.  The second step multiplication of the white-noise-free
kernel times ${\bf b}$ may be accomplished in $\mathcal{O}(JN)$ steps with this multiplication algorithm.
Note that for this particular example it is more efficient to compute $\bar f = {\bf y} -\sigma^2 {\bf I} {\bf b}$,
