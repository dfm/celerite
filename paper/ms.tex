...
t_n$.\footnote{We are using $t$ as the input coordinate because
...
``stationary'', \ie\ $k_\bvec{\alpha}(t_n,\,t_m)$ is only a function of
$\tau_{nm} \equiv |t_n - t_m|$.
...
Many astronomical objects have characteristic timescales determined by their
physical structure.
Variability is excited by noisy physical processes, and grows most strongly
at the characteristic timescale, but is also damped due to dissipation in the
system.
As a concrete example, stellar (asteroseismic) oscillations are strong at
resonant frequencies determined by the internal stellar structure, which are
...
\dfmtodo{One more sentence in conclusion.}
