...
%   - Description of APIs
% 4. Examples
%   - Simulated data: a simple exact celerite simulation, a celerite simulation with
...
evaluated in $\mathcal{O}(J^2\,N)$ operations \citep{Anderson:1999} where $J$
is the number of components in the mixture and $N$ is still the number of data
points.
...
marginalized likelihood in $\mathcal{O}(J^2\,N)$ operations.
...
g_{n,j} &=& \phi_{n,j}\,g_{n-1,j} + \phi_{n,j}\,x_n + \psi_{n,j}\,h_{n-1,j}\\
h_{n,j} &=& \phi_{n,j}\,h_{n-1,j} - \psi_{n,j}\,x_n - \psi_{n,j}\,g_{n-1,j}\\
u_{n,j} &=& \phi_{n+1,j}\,u_{n+1,j} + a_j\,x_{n+1} + \psi_{n+1,j}\,v_{n+1,j}\\
v_{n,j} &=& \phi_{n+1,j}\,v_{n+1,j} - b_j\,x_{n+1} - \psi_{n+1,j}\,u_{n+1,j}
\end{eqnarray}
...
for $K^{-1}\,\bvec{y}$ and $\det K$ in $\mathcal{O}(J^2\,N)$ operations.
...
$\bvec{x}_\mathrm{ext}$ and $\bvec{y}_\mathrm{ext}$ are defined schematically
as
\begin{eqnarray}
{\bvec{x}_\mathrm{ext}} ^\T =
\left(\begin{array}{cccccccccc}
    x_1 & u_{1,j} & v_{1,j} & g_{1,j} & h_{1,j} & x_2 & u_{2,j} & \cdots &
    h_{N-1,j} & x_N
\end{array}\right)
\end{eqnarray}
and
\begin{eqnarray}
{\bvec{y}_\mathrm{ext}} ^\T =
\left(\begin{array}{cccccccccc}
    y_1 & 0 & 0 & 0 & 0 & y_2 & 0 & \cdots &
    0 & y_N
...
for band matrices.
This decomposition can be used to compute the determinant of $K$ and to solve
$K^{-1}\,\bvec{y}$.
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/benchmark.pdf}
...
\caption{Benchmark\figurelabel{benchmark-all}}
\end{center}
\end{figure}
...
%\item Solution(s):\\
%  a). HODLR \citep{Ambikasaran:2013,Ambikasaran:2016}\\
%  b). CARMA (Lorentzian power spectra, but with limitations) \citep{Kelly:2014}\\
%  c). Tri-diagonal (Press \& Rybicki) \citep{Rybicki:1995}
%  d). KISS-GP \citep{Wilson:2015} %Kernel Interpolation for Scalable Structured Gaussian Processes (KISS-GP) Andrew Wilson, Hannes Nickisch Proceedings of The 32nd International Conference on Machine Learning, pp. 1775-1784, 2015
...
%   a). Inference of asteroseismic variability - ala Andrew Gordon Wilson \citep{Wilson:2013}, but
...
The power spectrum of a single exponential scales as $f^{-2}$ at high
frequency, while the Mat\'ern kernel scales as $f^{-4}$ which more
closely matches the power spectrum due to activity and granulation in
the Sun \citep{Aigrain:2004,Michel:2009}.
