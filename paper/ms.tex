...
This method, which we christen \celerite, can be applied to any Gaussian Process model where the spectral
density can be expressed as any general mixture of damped sinusoid functions.
We present a mathematical description of \celerite,
its relation to the stochastically-driven, damped simple harmonic oscillator,
and details of the implementation.  We give examples of \celerite\ applied to both
simulated and actual astronomical time-series, using it to estimate the time-series
power spectrum, while simultaneously accounting for correlated noise when inferring
model parameters.  These examples include Bayesian estimation of the strength
of stellar granulation, stellar rotation periods, asteroseismic spectra, and
transiting planet light curve parameters.  The model is flexible, fast, and
general, with potential for application to a wide range of astronomical problems,
for which we provide open-source software that implements \celerite\ in
\project{C++}, \project{Python}, and \project{Julia}.
...
We find that, while the vanilla banded solver scales as expected, the LAPACK
algorithm with tuned BLAS routines scales empirically as $\mathcal{O}(N\,J^2)$
empirically and offers the fastest solves for $J \gtrsim 8$ on all platforms
that we tested.
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=\textwidth]{../examples/benchmark/benchmark_darwin.pdf}
\caption{A benchmark showing the computational scaling of \celerite\ with a
    vanilla band solver as a function of the number of data points and the
    number of terms.
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=\textwidth]{../examples/benchmark/benchmark_darwin_lapack.pdf}
\caption{The same as \Figure{benchmark} but using the band solver from
    \project{LAPACK} with the \project{MKL} \project{BLAS} library.
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=\textwidth]{../examples/benchmark/benchmark_darwin_sparse.pdf}
\caption{The same as \Figure{benchmark} but using the \texttt{SparseLU} solver
...
curves from the \kepler\ mission \citep{Gilliland:2010,Chaplin:2011,Chaplin:2013,Stello:2013}
and asteroseismology is a
...
and they scale poorly to the current state-of-the-art datasets \citep{Brewer:2009}.
...
\citep{Brewer:2009,Corsaro:2014}, measurement of granulation noise \citep{Bastien:2013,Kallinger:2016},
