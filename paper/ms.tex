...
\citep{Gibson:2012}, and spectroscopic calibration
\citep{Czekala:2017,Evans:2015}.
...
discussed in the following \sectionname\ but we first discuss some
...
but we start with a discussion of some useful special cases of this model.
...
\citet{Rybicki:1995}.
To demonstrate this method, we write out the full system of
...
\end{pmatrix}\nonumber
\end{eqnarray}
where we follow \citet{Ambikasaran:2015} and call this the ``extended''
...
Neither of these properties be satisfied in general for multidimensional
...
extended matrix is still be banded with a bandwidth that scales linearly
...
We now turn to the application of the \celerite\ algorithm to astrophysical
variability.  Many astronomical objects have a characteristic timescale
which is determined by the physical structure.  Variability
is excited by noisy physical processes, which grows most strongly at
the characteristic timescale, but is also damped due to dissipation
in the system.  As a concrete example, stellar oscillations are strong at
resonant frequencies determined by the internal stellar structure,
which are both excited and damped by convective turbulence.
...
Each of these examples touches on an active area of research so we limit
our examples to be qualitative in nature and make no claim of optimality but
we hope that these examples encourage interested readers to investigate
...
framework \citep[see][for a potential application]{Czekala:2017}.
...
To demonstrate this method, we use a very simple heuristic model based on
where the model PSD is given by a mixture of 8 components with
...
Looking forward to \tess\ \citep{Ricker:2014,Campante:2016}, we infer
...
\caption{The posterior constraint on the rotation period of KIC~1430163 using
    ...
    autocorrelation function and the full \kepler\ baseline
    \citep{Mathur:2014}.
...
this section we explore some comparisons between \celerite\ and some
other popular methods and discuss some use cases where \celerite\ shines and
...
and the scaling is worse than \celerite\ so it, in general, be less
...
example,][and references therein]{Wilson:2015a} and we make no attempt to
...
as `flicker', $1/f$, or pink noise;  however, this particular type of noise
overpredicts the power spectrum at low and high frequency compared to the Solar
...
will be important for transit detection \citep{ForemanMackey:2015}, transit timing
\citep{Agol:2005,Holman:2005}, transit spectroscopy \citep{Brown:2001}, Doppler
beaming \citep{Loeb:2003,Zucker:2007}, tidal distortion \citep{Zucker:2007}, phase
...
spectrum \citep{Wilson:2013} avoids these difficulties:  it allows for the computation of
uncertainties, it accounts for the window function on the uncertainties of
the power spectrum, and in principle it can treat aliases if the full posterior
is explored.  One drawback is that the number of $J$ terms to include is
unknown, but may be inferred with computation of the evidence, or an
approximation such as the Bayesian information criterion \citep{Kelly:2014}.
In general, the parameters of the \celerite\ terms are more interesting due
to their physical meaning rather than the overall shape of the power spectrum,
as shown in the foregoing examples.
...
This work was performed in part under contract with the Jet Propulsion Laboratory (JPL)
funded by NASA through the Sagan Fellowship Program executed by the NASA Exoplanet Science Institute.
