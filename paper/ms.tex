...
\item The squared exponential kernel has the behavior of declining exponentially at large
separation, and has $c_1$ = 0.  An infinite number of exponential kernels may be used to
approximate a Gaussian kernel.  For example, to 
\item {\bf Mat\'ern 3/2 kernel.}  The Mat\'ern $3/2$ kernel is given by: 
\begin{equation}
K_{3/2}(\tau,\tau_0)
= (1+x)e^{-x},
\end{equation} 
where $x = \sqrt{3}\vert \tau/\tau_0\vert$.  This may be
well approximated by the sum of two exponential kernels: 
\begin{equation}
...
large values of $b$, the error on this scales as $2/(e^2b) \approx 0.27/b$; for
...
are too large.  One advantage of the Mat\'ern 3/2 kernel is that it is smooth across
...
timescales due to the sharpness of the kernel at $\tau=0$.  The Mat\'ern 3/2 kernel
turns out to be an excellent description of stellar granulation, and thus the approximate
formula is simultaneously physically relevant and computationally convenient.
\item Matern 5/2 kernel.
\item Cosine kernel
\item exp-sin-sq kernel
\end{itemize}
