...
performance algorithm by restricting our method to positive semi-definite
(PSD) matrices.
...
\subsection{Cholesky factorization of PSD semiseparable matrices}
...
\end{eqnarray}
where, since $K$ is PSD, $D_{n,n}>0$ for all $n$.
...
computing the determinant of PSD rank-$R$ semiseparable matrices.
As we will show below, with some care, this method can be used to compute
\eq{gp-likelihood} for \celeriteterm\ models.
In practice, when $K$ is PSD and this method is applicable, we find that this
method is about a factor of 20 faster than an optimized implementation of the
earlier, more general method from \citet{Ambikasaran:2015}.
...
i5 processor, but find similar performance on other platforms.
The \celerite\ implementation used for this test is the Python interface with
a C++ back end, but there is little overhead introduced by Python so we find
similar performance using implementations in C++ and Julia
\citep{Bezanzon:2012}.
...
\response{
For this demonstration, we randomly select a month-long segment of PDC data
...
To help clarify equations \ref{system}, we write out here an example of a semiseparable matrix with a
