...
will result in numerical overflow and underflow issues much like we found with
...
\begin{eqnarray}
f^+_{n,j} &=& \phi_{n+1,j}\,\left(f^+_{n+1,j} + \tilde{U}_{n,j}\,z_{n+1}\right) \\
f^-_{n,j} &=& \phi_{n,j}\,\left(f^-_{n-1,j} + \tilde{V}_{n-1,j}\,z_{n-1}\right) \\
y_n &=& A_{n,n}\,z_n + \sum_{j=1}^{2\,J} \left[ \tilde{V}_{n,j} f^+_{n,j} +
    \tilde U_{n-1,j} f^-_{n,j}\right] \quad,
\end{eqnarray}
where $f^+_{N,j} = 0$ and $f^-_{1,j} = 0$.
This algorithm requires two sweeps~--~$n=1, \ldots, N$ to compute $f^-$ and
$n=N,\ldots,1$ to compute $f^+$~--~with a scaling of $\mathcal{O}(N\,J)$.
