...
probabilistic inference that scale well with amount of data.
...
parameters.  The power spectrum of an irregularly sampled time series may be
estimated with Bayesian inference, with a posterior estimate of the associated
uncertainties, in principle avoiding the problems associated with window functions
and aliasing.
...
this section we will explore some comparisons between \celerite\ and some
...
%The general case with multiple \celerite\ terms can be checked using Sturm's
%theorem. The power spectrum is computed from taking the square of the Fourier
%transform of the data time series; hence, the power-spectrum must be non-negative.
%In constructing a kernel, if any of the parameters $a_j$ or $b_j$ are negative,
