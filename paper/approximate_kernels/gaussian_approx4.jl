...
for k=1:nexp
  ...
  c3 += x[1+(k-1)*3]*(x[2+(k-1)*3]^2-x[3+(k-1)*3]^2)
end
c2 = c2/c1
c3 = sqrt(abs((c3+c1*c2^2)/c1))
#model += c1.*exp(-c2.*time).*cos(x[nexp*3+1].*time)
model += c1.*exp(-c2.*time).*cos(c3.*time)
...
for k=1:nexp-1
  ...
  c3 += atrial[1+(k-1)*3]*(atrial[2+(k-1)*3]^2-atrial[3+(k-1)*3]^2)
end
c2 = c2/c1
c3 = sqrt((c3+c1*c2^2)/c1)
model += c1.*exp(-c2.*time).*cos(c3.*time)
clf()
plot(time,gaussian)
plot(time,model)
...
for k=1:nexp
  ...
  c3 += abest[1+(k-1)*3]*(abest[2+(k-1)*3]^2-abest[3+(k-1)*3]^2)
end
c2 = c2/c1
c3 = sqrt((c3+c1*c2^2)/c1)
model += c1.*exp(-c2.*time).*cos(c3.*time)
...
println("New minimum: ",chi," ",abest," ",c1," ",c2," ",c3," ",std(gaussian-model)," ",maximum(abs(gaussian-model)))
read(STDIN,Char)
